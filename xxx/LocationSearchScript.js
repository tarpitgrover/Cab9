...
app.directive('journeyBuilder', function () {
    var html = [
        '<div class="form-group" ng-repeat="wp in waypoints">',
        '   <input type="text" ng-model="wp.Text" placeholder="{{($first) ? \'From\' : (($last) ? \'To\' : \'Via\')}}" />',
        '</div>'
    ];
    return {
        restrict: 'A',
        ...
        link: function (scope, elem, attr) {
            ...
            scope.AddWaypoint = function (index) {
                scope.waypoints.splice(index, 0, new waypoint());
            }

            scope.RemoveWaypoint = function (waypoint) {
                var i = scope.waypoints.indexOf(waypoint);
